...
import { Form, FormControl, FormDescription, FormField, FormItem, FormLabel, FormMessage, FormLabel as ShadFormLabel } from "@/components/ui/form";
...
export default function AddNewContestPage() {
  ...
  const totalPrizePool = 0; // No individual prize amounts, only total prize pool
  const remainingPrize = form.watch("prizePool");
                      ...
                      {galleryImages.map((image, index) => (
                        <div key={index} className="relative group">
                          <div className="aspect-square rounded-lg overflow-hidden bg-muted">
                            <img src={URL.createObjectURL(image)} alt={`Gallery ${index + 1}`} className={`w-full h-full object-cover ${isDragActive ? "opacity-50" : "opacity-100"}`} />
                          </div>
                          <Button
                            type="button"
                            variant="destructive"
                ...
                <div className="flex items-center justify-between p-2 bg-muted rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className="text-xs">
                      <span className="text-xs font-semibold">${form.watch("prizePool").toLocaleString()}</span>
                    </Badge>
                  </div>
                </div>
              </div>
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between border-b pb-4">
                    <div>
                      ...
                      <FormDescription>Whether the contest is currently active.</FormDescription>
                    </div>
                    <FormControl>
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between border-b pb-4">
                    <div>
                      ...
                      <FormDescription>Make the contest visible to everyone.</FormDescription>
                    </div>
                    <FormControl>
                      <Switch checked={field.value} onCheckedChange={field.onChange} />
                    </FormControl>
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between">
                    <div>
                      ...
                      <FormDescription>Enable participants to vote for entries.</FormDescription>
                    </div>
                    <FormControl>
                      <Switch checked={field.value} onCheckedChange={field.onChange} />
                    </FormControl>
