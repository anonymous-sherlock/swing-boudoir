...
const PasswordInput = forwardRef<HTMLInputElement, PasswordInputProps>(
  ({ 
    ...
    const password = controlledValue !== undefined ? String(controlledValue) : internalPassword;
    ...
    // Check password validation and notify parent
    useEffect(() => {
      ...
      <div>
        ...
        {showStrengthIndicator && (
          <>
            ...
            >
              <div className={`h-full ${getStrengthColor(strengthScore)} transition-all duration-500 ease-out`} style={{ width: `${(strengthScore / 4) * 100}%` }}></div>
            </div>
            ...
            <ul className="space-y-1.5" aria-label="Password requirements">
              {strength.map((req, index) => (
                <li key={index} className="flex items-center gap-2">
                  {req.met ? <CheckIcon size={16} className="text-emerald-500" aria-hidden="true" /> : <XIcon size={16} className="text-muted-foreground/80" aria-hidden="true" />}
