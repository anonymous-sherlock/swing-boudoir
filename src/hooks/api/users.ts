...
const getUsers = async (params?: { page?: number; limit?: number; search?: string; role?: string; isActive?: boolean }): Promise<UsersResponse> => {
...
const updateNullUsernames = async (): Promise<{
  message: string;
  updatedCount: number;
  users: Array<{ id: string; email: string; oldUsername: string | null; newUsername: string }>;
}> => {
...
export const useGetUsers = (params?: { page?: number; limit?: number; search?: string; role?: string; isActive?: boolean }) => {
  return useQuery({
